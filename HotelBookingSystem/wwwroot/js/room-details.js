...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updatePriceDisplay(price, nights) {
    ...
    if (roomTotalElement)
      roomTotalElement.textContent = formatCurrency(roomTotal);
    if (totalPriceElement)
      totalPriceElement.textContent = formatCurrency(roomTotal);
  }
  ...
  function initializeScrollAnimations() {
    ...
    const animatedElements = document.querySelectorAll(
      ".room-description, .room-features-section, .room-specs, .room-policies, .room-reviews"
    );
  ...
  if (bookingForm) {
    bookingForm.addEventListener("submit", function (e) {
      ...
      if (!window.isUserAuthenticated) {
        ...
        const loginUrl = `${window.loginUrl}?returnUrl=${encodeURIComponent(
          bookingUrl
        )}`;
